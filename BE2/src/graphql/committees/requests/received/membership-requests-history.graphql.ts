...
export const receivedRequestsHistoryTypes = `
  type CommitteeMembershipRequestHistoryItem {
    committeeId: Int!
  ...
  type ReceivedRequestsHistoryResponse {
    data: [CommitteeMembershipRequestHistoryItem!]!
  }
`;

export const receivedRequestsHistoryQueryFields = `
  receivedRequestsHistory: ReceivedRequestsHistoryResponse!
  receivedRequestsHistoryByUserId(userId: Int!, committeeId: Int!): ReceivedRequestsHistoryResponse!
`;

export const receivedRequestsHistoryResolvers = {
  Query: {
    async receivedRequestsHistory(_: any, __: any, context: any) {
    ...
    async receivedRequestsHistoryByUserId(_: any, args: { userId: number; committeeId: number }, context: any) {
      const authHeader = context.headers?.authorization;
      const tokenFromCookie = context.cookies?.token;
      let accessToken: string | null = null;
      ...
      const rows = await query<any[]>(
        `SELECT
         ...
         ORDER BY crr.requested_at DESC`,
         [loggedInUserId, targetUserId, targetCommitteeId]
      );
