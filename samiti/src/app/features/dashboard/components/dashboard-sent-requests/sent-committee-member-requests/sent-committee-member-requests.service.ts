...
export interface SentCommitteeMemberRequestItem {
  committeeId: number;
...
export class SentCommitteeMemberRequestsService {
  ...
  getSentCommitteeMemberRequests(): Observable<SentCommitteeMemberRequestItem[]> {
    const query = `query SentCommitteeMemberRequests {
      sentCommitteeMemberRequests {
        data {
          committeeId
          committeeName
          committeeLogo
    ...
    return this.http.post<GraphQLResponseEnvelope<{ sentCommitteeMemberRequests: { data: SentCommitteeMemberRequestItem[] } }>>(this.graphqlUrl, { query }).pipe(
      map((response) => {
        return this.unwrapDataArray(
          {
            data: response.data?.sentCommitteeMemberRequests,
            errors: response.errors
          },
          'Failed to fetch sent member requests'
        ).map((item) => ({
          ...item,
